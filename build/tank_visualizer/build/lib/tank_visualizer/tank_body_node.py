...
# ADDED 'Twist' TO THE LINE BELOW SO ROS 2 CAN USE THE KEYBOARD DATA
...
class TankBodyVisualizer(Node):
    def __init__(self):
        ...
        # --- KEYBOARD CONTROLS STATE & SUBSCRIPTION ---
        self.linear_velocity = 0.0
        self.angular_velocity = 0.0
        self.create_subscription(Twist, '/cmd_vel', self.cmd_vel_callback, 10)

        # Tank dimensions (Restored because these are required to draw the shapes)
    ...
    # --- NEW CALLBACK TO CAPTURE KEYBOARD INPUT ---
    def cmd_vel_callback(self, msg):
        self.linear_velocity = msg.linear.x    # Forward/Backward
        self.angular_velocity = msg.angular.z  # Turn rate

    def timer_callback(self):
        time_now = self.get_clock().now().to_msg()

        # --- CALCULATE POSITION FROM KEYBOARD VELOCITY ---
        dt = 0.05 
        self.theta += self.angular_velocity * dt
        self.x += self.linear_velocity * math.cos(self.theta) * dt
        self.y += self.linear_velocity * math.sin(self.theta) * dt

        # --- Broadcast tank_body TF ---
        t = TransformStamped()
        ...
        # --- Broadcast tank_turret TF (independent rotation) ---
        ...
        # C. Add 6 wheels (3 left, 3 right)
        x_positions = [-0.3, 0.0, 0.3]
        wheel_id = 1
        for x_pos in x_positions:
            add_wheel(wheel_id, x_pos, 1)   # Right
            wheel_id += 1
            add_wheel(wheel_id, x_pos, -1)  # Left
            wheel_id += 1
